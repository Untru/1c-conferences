...
help: ## Показать справку по командам
	...
	@echo "  make github-deploy   - Показать инструкции по деплою"
	@echo "  make deploy-with-nav - Обновить навигацию и создать деплой"
	@echo ""
...
github-deploy: ## Запустить деплой вручную
	...
	@echo "  1. 🗂️ Обновит навигацию (если нужно)"
	@echo "  2. 🔨 Соберет сайт"
	@echo "  3. 🚀 Задеплоит на GitHub Pages"
	@echo ""
	@echo "$(GREEN)Команды для деплоя:$(NC)"
	@echo "  git add ."
	@echo "  git commit -m '✨ Добавлено новое событие'"
	@echo "  git push origin main"
	@echo ""
	@echo "$(YELLOW)💡 Совет: Навигация обновится автоматически!$(NC)"

deploy-with-nav: ## Обновить навигацию и создать коммит для деплоя
	@echo "$(BLUE)🚀 Подготавливаю деплой с обновлением навигации...$(NC)"
	@$(MAKE) nav
	@echo ""
	@echo "$(YELLOW)📝 Создаю коммит с обновленной навигацией...$(NC)"
	@if [ -n "$$(git status --porcelain)" ]; then \
		git add .; \
		echo "Введите сообщение коммита (или нажмите Enter для '✨ Обновление событий и навигации'):"; \
		read commit_msg; \
		if [ -z "$$commit_msg" ]; then \
			commit_msg="✨ Обновление событий и навигации"; \
		fi; \
		git commit -m "$$commit_msg"; \
		echo "$(GREEN)✅ Коммит создан: $$commit_msg$(NC)"; \
		echo "$(BLUE)🚀 Пушу изменения...$(NC)"; \
		git push origin main; \
		echo "$(GREEN)🎉 Деплой запущен! Проверьте GitHub Actions.$(NC)"; \
	else \
		echo "$(YELLOW)⚠️ Нет изменений для коммита$(NC)"; \
	fi
